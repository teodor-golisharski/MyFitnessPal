#pragma once
#include <string>

const std::string USERNAME_IS_TAKEN = "Username is taken!";
const std::string FILE_NOT_FOUND = "Error! File was not found.";
const std::string INVALID_USERNAME = "Invalid username. Please try again.";
const std::string INVALID_PASSWORD = "Invalid password. Please ensure it meets the requirements.";
const std::string INVALID_AGE = "Invalid age. Please enter a valid age (e.g., 14-100).";

std::string ACCOUNT_CREATED_SUCCESSFULLY = "Account created successfully! Welcome to MyFitnessPal, ";

